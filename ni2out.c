...
{
    fprintf(stderr, "usage: %s [--list] [--delay] [-i <inputfile>] [-s SID]\n", psz);
    exit(EXIT_FAILURE);
}
...
int feed_x_pad(uint8_t *data_ptr, uint8_t ci_flag, ni2http_channel_t *chan, uint8_t x_pad_ind, int start_pos)
{
	...
	if(x_pad_ind == 1) {
		...
		if(fill_dls_pad(data_ptr, ci_flag ? 3 : 4, data_ptr[3] == 0x02 && ci_flag, chan)){
			INFO("sid[%#04x] song: %s", chan->sid, chan->pad_data);
		}
	}
...
{   
    ...
    while ((c = getopt_long(i_argc, ppsz_argv, "i:s:ldh", long_options, NULL)) != -1)
    {
        switch (c) {
        case 'i':
	...
	if (chan->sid == 0 && config_parsed == 1) {
		fprintf(stderr,"Error parsing parameters: invalid SID\n");
		exit(-1);
	}
...
//					strncpy(channels[ch]->name, s_data->label, sizeof(s_data->label));  // code will result in a compiling error
				...
				for (idx=0; idx < fc.nst; idx++) {
					...
					if(sstc.sad == s_data->pa->startaddr){
						...
						INFO("sid[%#04x]: channel_map[%d] = channel[%d]", s_data->sid, s_data->pa->startaddr, idx);
						idx_found=1;
						channels_mapped++;
						...
						if(strlen(channels[ch]->file_name) > 0) {
							...
							if(!channels[ch]->file) {
								INFO("sid[%#04x]: can't open output dump file %s.", s_data->sid, channels[ch]->file_name);
							} else {
								int fd_o = fileno(channels[ch]->file);
								if (fcntl(fd_o, F_SETFL, fcntl(fd_o, F_GETFL) | O_NONBLOCK) == -1) {
									INFO("sid[%#04x]: can't set non-block output for file %s.", s_data->sid, channels[ch]->file_name);
								}
							}
